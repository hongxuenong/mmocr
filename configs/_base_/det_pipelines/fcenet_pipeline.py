...
# for shopee dataset
leval_prop_range_shopee = ((0, 0.25), (0.2, 0.65), (0.55, 1.0))
train_pipeline_shopee = [
    dict(type='LoadImageFromFile', color_type='color_ignore_orientation'),
    dict(
        type='LoadTextAnnotations',
        with_bbox=True,
    ...
    dict(
        type='FCENetTargets',
        fourier_degree=5,
        level_proportion_range=leval_prop_range_shopee),
    dict(
        type='CustomFormatBundle',
        keys=['p3_maps', 'p4_maps', 'p5_maps'],
        visualize=dict(flag=False, boundary_key=None)),
...
img_scale_shopee = (1080, 736)
test_pipeline_shopee = [
    dict(type='LoadImageFromFile', color_type='color_ignore_orientation'),
    dict(
        type='MultiScaleFlipAug',
        img_scale=img_scale_shopee,
        flip=False,
        transforms=[
            dict(type='Resize', img_scale=(1280, 800), keep_ratio=True),
            dict(type='Normalize', **img_norm_cfg),
